...
@router.get("/list")
def get_group_list(accountinfo: Annotated[p_models.Account, Depends(handle_get_current_user_oauth2)]):
    result_list = []
    list_personal_participant = handle_get_participants_by_account(accountinfo.id)
...
def get_message_list(accountinfo: Annotated[p_models.Account, Depends(handle_get_current_user_oauth2)], groupid: str):
    valid_participant = handle_get_participant_by_account_and_group(accountinfo.id, groupid)
...
def get_group_participant_list(accountinfo: Annotated[p_models.Account, Depends(handle_get_current_user_oauth2)],
                               group_id: str):
    valid_participant = handle_get_participant_by_account_and_group(accountinfo.id, group_id)
    if valid_participant is not None:
        result_list = []
...
def create_new_group(data: p_schemas.AccountSchema,
                     accountinfo: Annotated[p_models.Account, Depends(handle_get_current_user_oauth2)]):
    try:
        created_group = handle_create_new_group([data.id, accountinfo.id])
